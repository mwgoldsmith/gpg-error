...
static int parse_config_h(const char* fname) {
  FILE* fp;
  char line[LINESIZE];
  int lnr = 0;
  char* p1;
  ...
  fp = fopen(fname, "r");
  if (!fp) {
    fprintf(stderr, "%s:%d: can't open file: %s", fname, lnr, strerror(errno));
    return 1;
  }
...
static void write_str(char* line) {
...
static void write_sources_or_codes(char* line) {
  static int in_intro;
  char *p1, *p2;
...
/* Write system errnos to stdout.  The function is allowed to
   modify LINE.  Trailing white space is already removed.  Passing
   NULL resets the internal state.  */
static void write_errnos_in(char* line) {
  static int state;
  char *p1, *p2;
...
/* Create the full file name for NAME and return a newly allocated
   ...
   replace '&' with REPL. */
static char* mk_include_name(const char* name, const char* repl) {
  char *incfname, *p;
  const char* s;

  incfname = malloc(strlen(srcdir) + strlen(name) + (repl ? strlen(repl) : 0) + 1);
  if (!incfname) {
    fputs(PGM ": out of core\n", stderr);
    exit(1);
  }
...
static void include_file(const char* fname, int lnr, const char* name, void (*outf)(char*)) {
  FILE* fp;
  char* incfname;
  int inclnr;
  char line[LINESIZE];
...
/* Try to include the file NAME.  Returns true if it does not
   exist. */
static int try_include_file(const char* fname, int lnr, const char* name, void (*outf)(char*)) {
  int rc;
  char* incfname;
  int repl_flag;
...
static int write_special(const char* fname, int lnr, const char* tag) {
  if (!strcmp(tag, "version")) {
    ...
    if (!replacement_for_off_type) {
      ...
      if (!strcmp(replacement_for_off_type, "int64_t") && !stdint_h_included && have_stdint_h) {
  ...
  while (fgets(line, LINESIZE, fp)) {
    ...
    *p2++ = 0;
    ...
    if (!strcmp(p1, "configure_input")) {
      s = strrchr(fname, '/');
      printf("Do not edit.  Generated from %s for %s.", s ? s + 1 : fname, host_triplet);
      fputs(p2, stdout);
